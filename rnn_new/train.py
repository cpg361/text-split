...
class Encoder(object):
    ...
    def __init__(self,**kwargs):
        ...
        with tf.variable_scope("BIRNN_ENCODER"):
            self.encoder = BidirectionalEncoder(self.embed_num,self.nhids_num, name = 'birnn_encoder')
    ...
    def nmt(self,src,src_mask,src_label) :
        tf_sents=tf.transpose(src,perm=[1,0,2])
    ...
    def build_trainer(self):
        self.src = tf.placeholder(dtype=tf.float32,shape=(None,self.seq_len,self.embed_num),name="src_holder_train")
    ...
    def build_sampler(self):
        ...
        self.src_sample=tf.placeholder(dtype=tf.float32, shape=(1,None,self.embed_num),name="src_holder_sample")
        self.src_mask_sample=tf.placeholder(dtype=tf.float32,shape=(1,None),name="src_mask_sample")
        #tf_sample = tf.transpose(self.src_sample,perm=[1,0,2])
        #tf_mask_sample = tf.transpose(self.src_mask_sample,perm=[1,0])
...
class GRU(object):
    ...
    def apply(self,state_below,mask_below=None,init_state=None):
        n_steps =int(state_below.shape[0])
...
class Sampler(object):
    ...
    def apply(self,session,sent_batch,sent_mask_batch,sent_label_batch):
        batch_size = int(sent_batch.shape[0])
        ...
        for i in range(hook_samples):
            ...
            prob=self.sample_fn(session,(input_[i][None,:,:]),(numpy.array(input_mask[i]).flatten())[None,:])
            ...
            order=order.flatten()
            print('target:',list(target_label))
...
class Test(object):
    ...
    def apply(self,session,data_stream,out_file):
        ...
        for sent in data_stream:
            i=i+1
            out_split=[]
            sent_mask = [0. if all(unit) else  1. for unit in sent]
            #prob=self.encoder.step_sample(session,(numpy.array(sent).flatten().T)[:,None],(numpy.array(sent_mask).flatten())[:,None])
            prob=self.encoder.step_sample(session,(numpy.array(sent).reshape(1,len(sent),self.embed_num)),(numpy.array(sent_mask).flatten())[None,:])
            #order=numpy.argmax(prob,axis = -1)
            #order=order.flatten()
